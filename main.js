...
let afkCommand = new Command("!afk", (player, message) => {
    let afkIndex = afkPlayers.indexOf(player);
    if (afkIndex != -1) {
        afkPlayers.splice(afkIndex, 1);
        activePlayers.push(player);
        room.sendAnnouncement(`You not afk now!`, player.id, orangeColor, 'bold', 0);
    }
    else {
        //потрібно перевірити чи гравець грає в якійсь команді і так далі
        if (player.team == 1){
            afkIndex = teamRed.indexOf(player);
            teamBlue.splice(afkIndex, 1);
            afkPlayers.push(player);
            room.setPlayerTeam(player.id, 0);
        } else if (player.team == 2) {
            teamRed.splice(afkIndex, 1);
            afkPlayers.push(player);
            room.setPlayerTeam(player.id, 0);
        } else {
            afkIndex = activePlayers.indexOf(player);
            activePlayers.slice(afkIndex, 1);
            afkPlayers.push(player);
        }
    }
    updateGame();
...
let afksCommand = new Command("!afks", (player, message) => {
    let namesString = '';
    afkPlayers.forEach(player => {
        namesString += player.name + ', ';
    });
    namesString = namesString.slice(0, -2);
    room.sendAnnouncement(`Players afk: ${namesString}`, player.id, orangeColor, 'bold', 0);
}, false);
...
room.onPlayerLeave = function (player) {
    console.log("Start removing player");
    //потрібно перевірити чи гравець не був афк і видалити його зі списку афк, якщо був
    updateGame();
}
...
function shuffleTeams() {
    let shuffled = room.getPlayerList()
    ...
    let teamSize = Math.floor(shuffled.length / 2);

    for(let i = 0; i < teamSize; i++) {
        room.setPlayerTeam(shuffled[i].id, 1);
        room.setPlayerTeam(shuffled[i+teamSize].id, 2);
    }
}
...
function updateGame(){
    ...
    let activePlayers = players.filter(item => item.team == 0);
    let teamBluePlayers = players.filter(item => item.team == 2);
    let teamRedplayers = players.filter(item => item.team == 1);
 
    if (teamRedplayers.length == 0 && teamBluePlayers.length == 0 && activePlayers.length == 1) {
        console.log("block1");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
    ...
    if ((teamBluePlayers.length + teamRedplayers.length) == 1 && activePlayers.length == 0) {
        console.log("block2");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
        setMapNoPlayersFutsal();
        if (teamBluePlayers.length == 1) {
            room.setPlayerTeam(teamBluePlayers[0].id, 1);
        }
        room.startGame();
        return; 
    }

    if (teamRedplayers.length == 1 && teamBluePlayers.length == 0 && activePlayers.length == 1) {
        console.log("block3");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
        setMap1v1Futsal();
        room.setPlayerTeam(activePlayers[0].id, 2);
        room.startGame();
        return; 
    }

    if (teamRedplayers.length == teamBluePlayers.length && teamRedplayers.length == 1 && activePlayers.length >= 2) {
        console.log("block4");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
        setMap1v1Futsal();
        room.setPlayerTeam(activePlayers[0].id, 1);
        room.setPlayerTeam(activePlayers[1].id, 2);
        room.startGame();
        return; 
    }

    if (teamRedplayers.length == teamBluePlayers.length && teamRedplayers.length < teamPlayerLimit && activePlayers.length >= 2) {
        console.log("block5");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
        setMap4v4Futsal();
    ...
    if (teamRedplayers.length < teamBluePlayers.length && activePlayers.length == 0) {
        console.log("block6");
        console.log(`teamRedCount=${teamRedplayers.length}, teamBlueCount=${teamBluePlayers.length}, activePlayersCount=${activePlayers.length}`);
        room.stopGame();
        if (teamRedplayers.length <= 2) setMap1v1Futsal();
        else setMap4v4Futsal();
        room.setPlayerTeam(teamBluePlayers[teamBluePlayers.length - 1].id, 0);
        room.startGame();
        return; 
    }
    ...
    if (teamRedplayers.length > teamBluePlayers.length && activePlayers.length == 0) {
        ...
        room.setPlayerTeam(teamRedplayers[teamRedplayers.length - 1].id, 0);
        room.startGame();
        return; 
    }
    if (teamRedplayers.length > teamBluePlayers.length && activePlayers.length != 0) {
...
function playerToSpectators() {
    let playersInTeams = room.getPlayerList().filter(p => p.team != 0);
    playersInTeams.forEach(element => {
        room.setPlayerTeam(element.id, 0);
    });
}
